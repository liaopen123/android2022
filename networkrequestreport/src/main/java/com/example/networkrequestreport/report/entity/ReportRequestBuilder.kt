...
class ReportRequestBuilder(chain: Interceptor.Chain) : IRequest {
    ...
    fun build()=ReportRequest(startTime(),protocol(),url(),method(),contentLength(),contentType(),body(),headers())
   }